...
#     if ret == False:
#         break
#     cv2.imwrite('./results/frames/frame'+str(i)+'.jpg',frame)
#     i+=1
...
def getTime(frameNumber):
	...
	minutes = minutes%60
	totalTime = str("{0:02}".format(hours))+":"+str("{0:02}".format(minutes))+":"+str("{0:02}".format(seconds))+"."+str(milliseconds)
	return totalTime
...
def findSmoothFramesInVideo(csvFilePath,frameFolder,outputFolder):
	...
	for j in range(2,numOfScenes):
		...
		t = getTime(min_i)
		print(min_i,t)
		finalData = finalData.append(pd.DataFrame([[min_i,t,min_i/30]],columns = ["frame","time","timeinseconds"]),ignore_index=True)
